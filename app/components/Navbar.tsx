...
const liveBadgeClass =
    'text-[9px] px-1.5 py-0 h-5 shrink-0 bg-emerald-100 text-emerald-700 border-emerald-200 shadow-[0_0_8px_rgba(16,185,129,0.3)]';

const platformItems = [
    { label: 'Elaris Creative', description: 'Agentic infrastructure for brand-safe advertising at scale.', href: STUDIO_URL, badge: 'LIVE' as const },
    { label: 'Creator Mode (UGC)', description: 'AI-generated avatars & user-style content', href: STUDIO_URL, badge: 'LIVE' as const },
    { label: 'Audio Intelligence', description: 'Text-to-speech & multi-lingual dubbing', href: STUDIO_URL, badge: 'LIVE' as const },
];
            ...
            <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
                                            ...
                                            >
                                                <div className="flex items-start justify-between gap-2">
                                                    ...
                                                    {item.badge && (
                                                        <Badge
                                                            variant="secondary"
                                                            className={item.badge === 'LIVE' ? liveBadgeClass : 'text-[9px] px-1.5 py-0 h-5 shrink-0 bg-gray-100 text-gray-500 border-gray-200'}
                                                        >
                                                            {item.badge}
                                                        </Badge>
                                                    )}
                ...
                <div className="hidden md:block">
                    <Button size="sm" asChild className="h-9 rounded-full bg-gray-900 hover:bg-black text-white text-[13px] px-5 shadow-lg shadow-gray-900/20 hover:shadow-purple-500/20 transition-all duration-300 border border-white/10">
                        <a href="https://calendly.com/sarthak-bhardwaj-elarislabs/30min" target="_blank" rel="noopener noreferrer">Book a Demo</a>
                    </Button>
                </div>
                ...
                {isMobileMenuOpen && (
                    ...
                    >
                        <div className="px-6 py-6 flex flex-col gap-4">
                            ...
                            {platformItems.map((item) => (
                                <a key={item.label} href={item.href} target={item.href.startsWith('http') ? '_blank' : undefined} rel={item.href.startsWith('http') ? 'noopener noreferrer' : undefined} className="flex items-center justify-between text-[14px] text-gray-700 hover:text-purple-600 py-1 gap-2" onClick={() => setIsMobileMenuOpen(false)}>
                                    <span className="min-w-0">{item.label}</span>
                                    {item.badge && (
                                        <Badge variant="secondary" className={item.badge === 'LIVE' ? liveBadgeClass : 'text-[9px] h-4 px-1 bg-gray-100 text-gray-500 border-gray-200'}>
                                            {item.badge}
                                        </Badge>
                                    )}
                                </a>
